...
class Hdf5Database(Database):
    ...
    def dataset_names(self):
        return [d.name for d in self.datasets_]

    def has_dataset(self, dataset_name):
        """ Checks whether or not a dataset with the given name exists.
...
class Hdf5Dataset(Dataset):
    ...
    def metadata(self):
        ...
        """
        if METADATA_KEY in self.data_.keys():
    ...
    def convex_piece_data(self, key):
        if CONVEX_PIECES_KEY in self.objects[key].keys():
    ...
    def metadata_data(self, metadata):
        if metadata in self.metadata.keys():
    ...
    def graspable(self, key):
        ...
        if self.convex_piece_data(key) is not None:
            convex_pieces = []
            for piece_key in self.convex_piece_data(key).keys():
    ...
    def delete_graspable(self, key):
        ...
        del self.object(key)[LOCAL_FEATURES_KEY]
        del self.object(key)[GLOBAL_FEATURES_KEY]
    ...
    def convex_pieces(self, key):
        ...
        # read convex pieces
        convex_pieces = []
        for convex_piece_key in self.convex_piece_data(key).keys():
    ...
    def metric(self, metric_name):
        ...
        for key in metric_group.keys():
            metric_config[key] = {}
            for k, v in metric_group[key].attrs.iteritems():
                metric_config[key][k] = v
        for key, value in metric_group.attrs.iteritems():
    ...
    def available_metrics(self, key, gripper='pr2'):
        ...
        metrics = set()
        for grasp in grasps:
            metrics.update(gm[grasp.id].keys())
        return list(metrics)
    ...
    def grasp_metrics(self, key, grasps, gripper='pr2', stable_pose_id=None):
        ...
        key : :obj:`str`
            key of object to check metrics for
        grasps : :obj:`list`
            list of grasps to return metrics for
        gripper : :obj:`str`
            name of gripper
        stable_pose_id : :obj:`str`
            id of stable pose
    ...
    def delete_grasp_metrics(self, key, grasps, metric_name, gripper='pr2'):
        """ Deletes the grasp metrics with the given name.
        ...
        if gripper not in self.grasp_data(key).keys():
            ...
            return False
        for grasp in grasps:
            grasp_key = GRASP_KEY + '_' + str(grasp.id)
            metric_names = self.grasp_data(key)[gripper][grasp_key][GRASP_METRICS_KEY].attrs.keys()
            del_metric_names = [m for m in metric_names if m.startswith(metric_name)]

            for m in del_metric_names:
                del self.grasp_data(key)[gripper][grasp_key][GRASP_METRICS_KEY].attrs[m]
        return True
    ...
    def grasp_features(self, key, grasps, gripper='pr2', stable_pose_id=None):
        """ Returns the features associated with the grasps for the given graspable, optionally associated with the given stable pose.

        Parameters
        ----------
        key : :obj:`str`
            key of object
        grasps : :obj:`list` of :obj:`dexnet.grasping.ParallelJawPtGrasp3D`
            grasps to return features for
        gripper : :obj:`str`
            name of gripper
        stable_pose_id : :obj:`str`
            id of stable pose
        ...
        Returns
        -------
        :obj:`list` of :obj:`GraspFeature`
            features associated with the given object and grasps
        """
        if gripper not in self.grasp_data(key).keys():
            logging.warning('Gripper type %s not found. Returning empty list' %(gripper))
            return {}
        return Hdf5ObjectFactory.grasp_features(grasps, self.grasp_data(key, gripper))

    def store_grasp_features(self, key, grasp_feature_dict, gripper='pr2', stable_pose_id=None, force_overwrite=False):
        """ Add grasp metrics in list metrics to the data associated with grasps.
        ...
        key : :obj:`str`
            key of object
        grasp_feature_dict : :obj:`dict` mapping :obj:`int` to :obj:`list` of :obj:`GraspFeature`
            grasp features to store
        gripper : :obj:`str`
            name of gripper
        stable_pose_id : :obj:`str`
            id of stable pose
        ...
        -------
        bool
            True if successful store, False otherwise
        """
        return Hdf5ObjectFactory.write_grasp_features(grasp_feature_dict, self.grasp_data(key, gripper), force_overwrite)
    ...
    def rendered_images(self, key, stable_pose_id=None, render_mode=None):
        ...
        if stable_pose_id is not None and RENDERED_IMAGES_KEY not in self.stable_pose_data(key)[stable_pose_id].keys():
            ...
            return []
        if stable_pose_id is not None and render_mode not in self.rendered_image_data(key, stable_pose_id).keys():
        ...
        if stable_pose_id is None and RENDERED_IMAGES_KEY not in self.object(key).keys():
            ...
            return []
        if stable_pose_id is None and render_mode not in self.rendered_image_data(key).keys():
    ...
    def rendered_image_types(self, key, stable_pose_id=None):
        ...
        if stable_pose_id is None:
            return self.rendered_image_data(key).keys()

        render_modes = self.rendered_image_data(key, stable_pose_id).keys()
        return render_modes
    ...
    def attach_metadata_func(self, metadata_name, metadata_func, overwrite=False, store_func=True):
        ...
        if store_func:
            metadata_group = self.metadata_data(metadata_name)
            bits_per_float = np.dtype(np.float32).itemsize * BYTESIZE
            try:
                serialized = dill.dumps(metadata_func)
                serialized = serialized.ljust(len(serialized) + bits_per_float - len(serialized) % bits_per_float, '\x00')
                serialized = np.fromstring(serialized, dtype=np.float32)
            except:
                logging.warning('Failed to serialize function. Aborting')
                return False
            if METADATA_FUNC_KEY in metadata_group.keys():
    ...
    def metadata_func(self, metadata_name):
        ...
        if not self.has_metadata(metadata_name):
            ...
            return None
        if metadata_name in self._metadata_functions.keys():
