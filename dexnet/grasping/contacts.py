...
class Contact3D(Contact):
    ...
    """
    def __init__(self, graspable, contact_point, in_direction, normal=None):
        ...
        self.normal_ = normal # outward facing normal
    ...
    def _compute_normal(self):
        ...
        """
        # compute normal using trimesh
        _, normal = self.graspable.mesh.find_contact(self.point, self.in_direction)
        if normal is None:
            if self.graspable.sdf is None:
                raise ValueError('Contact normal is undefined!')

            # tf to grid
            pt_grid = self.graspable.sdf.transform_pt_obj_to_grid(self.point)

            # compute outward facing normal from SDF
            try:
                normal = self.graspable.sdf.surface_normal(pt_grid)
                normal = self.graspable.sdf.transform_pt_grid_to_obj(normal, direction=True)
            except:
                logging.debug('Failed to compute surface normal!')
                return

        # flip normal to point outward if in_direction is defined
        if np.dot(self.in_direction, normal) > 0:
            normal = -normal
        self.normal_ = normal

    def tangents(self, direction=None, align_axes=False, max_samples=1000):
    ...
    def surface_information(self, width, num_steps, sigma_range=0.1, sigma_spatial=1,
        ...
        proj_window = self.surface_window_projection(width, num_steps,
                                                     ...
                                                     back_up=back_up, max_projection=max_projection, samples_per_grid=samples_per_grid,
