...
class PointGrasp(Grasp):
    ...
    #NOTE: implementation of find_contacts must return success, array of contacts (one per column)

class Grasp2D(object):
    """
    Struct to hold 2D grasps for use in image space.
    """
    def __init__(self, center, angle, width=0):
        self.center = center
        self.angle = angle
        self.width = width

    def gripper_pose(self, depth, T_camera_world, ir_intrinsics):
        """ Compute a gripper pose given a depth and camera registration """
        # get grasp parameters in image basis
        grasp_center_im = np.array([self.center[1], self.center[0]])
        grasp_theta_im = self.angle
        grasp_axis_im = np.array([np.cos(grasp_theta_im), np.sin(grasp_theta_im)])
        ...
        grasp_center_camera = ir_intrinsics.deproject_pixel(depth, center_px_im)
        grasp_center_camera = grasp_center_camera.data

        # convert to pose (assuming YuMi gripper)
        grasp_rot_camera, _, _ = np.linalg.svd(grasp_axis_camera.reshape(3,1))
        grasp_x_camera = np.array([0,0,1])
        grasp_z_camera = np.cross(grasp_x_camera, grasp_axis_camera)
        grasp_rot_camera = np.array([grasp_x_camera, grasp_axis_camera, grasp_z_camera]).T
...
class ParallelJawPtGrasp3D(PointGrasp):
    ...
    def __init__(self, configuration, frame='object', grasp_id=None):
        ...
        grasp_center, grasp_axis, grasp_width, grasp_point1, grasp_point2, grasp_normal1, grasp_normal2, grasp_angle, jaw_width, min_grasp_width = \
        ...
        self.grasp_point1_ = grasp_point1
        self.grasp_point2_ = grasp_point2
        self.grasp_normal1_ = grasp_normal1
        self.grasp_normal2_ = grasp_normal2
    ...
    @property
    def grasp_point1(self):
        """ :obj:`numpy.ndarray` : 3-vector specifying the center of the jaws """
        return self.grasp_point1_

    @property
    def grasp_point2(self):
        """ :obj:`numpy.ndarray` : 3-vector specifying the center of the jaws """
        return self.grasp_point2_

    @property
    def grasp_normal1(self):
        """ :obj:`numpy.ndarray` : 3-vector specifying the center of the jaws """
        return self.grasp_normal1_

    @property
    def grasp_normal2(self):
        """ :obj:`numpy.ndarray` : 3-vector specifying the center of the jaws """
        return self.grasp_normal2_
    ...
    @property
    def configuration(self):
        """ :obj:`numpy.ndarray` : vector specifying the parameters of the grasp as follows (grasp_center, grasp_axis, grasp_angle, grasp_width, jaw_width) """
        return ParallelJawPtGrasp3D.configuration_from_params(self.center_, self.axis_, self.max_grasp_width_, self.approach_angle_, self.jaw_width_, self.min_grasp_width_)
    ...
    def distance(g1, g2, alpha=0.05):
        ...
        dot_prod = max(min(np.abs(g1.axis.dot(g2.axis)), 1.0), 0.0)
    ...
    @staticmethod
    def configuration_from_params(center, axis, width, contact_point1, contact_point2, contact_normal1, contact_normal2, angle=0, jaw_width=0, min_width=0):
        ...
        configuration = np.zeros(22)
        configuration[0:3] = center
        configuration[3:6] = axis
        configuration[6] = width
        configuration[7:10] = contact_point1
        configuration[10:13] = contact_point2
        configuration[13:16] = contact_normal1
        configuration[16:19] = contact_normal2
        configuration[19] = angle
        configuration[20] = jaw_width
        configuration[21] = min_width
    ...
    def params_from_configuration(configuration):
        ...
        """
        if not isinstance(configuration, np.ndarray) or (configuration.shape[0] != 21 and configuration.shape[0] != 22):
            raise ValueError('Configuration must be numpy ndarray of size 21 or 22')
        if configuration.shape[0] == 21:
            min_grasp_width = 0
        else:
            min_grasp_width = configuration[21]
        if np.abs(np.linalg.norm(configuration[3:6]) - 1.0) > 1e-5:
            raise ValueError('Illegal grasp axis. Must be norm one')
        return configuration[0:3], configuration[3:6], configuration[6], configuration[7:10], configuration[10:13], configuration[13:16], configuration[16:19], configuration[19], configuration[20], min_grasp_width
    ...
    def grasp_from_endpoints(g1, g2, width=None, approach_angle=0, close_width=0):
        ...
        return ParallelJawPtGrasp3D(ParallelJawPtGrasp3D.configuration_from_params(x, v, width, min_width=close_width, angle=approach_angle))
    ...
    def unrotated_full_axis(self):
        ...
        # # x forward to z forward
        # R = np.array([[0,1,0],[0,0,1],[1,0,0]]).dot(np.c_[grasp_axis_x, np.c_[grasp_axis_y, grasp_axis_z]])
    ...
    def T_obj_grasp(self):
        ...
        """
        T_obj_grasp = RigidTransform(self.rotated_full_axis, self.center, from_frame='grasp', to_frame='obj')
        return T_obj_grasp
    ...
    def gripper_pose(self, gripper=None):
        ...
        """
        if gripper is None:
            T_grasp_gripper = RigidTransform(from_frame='gripper', to_frame='grasp')
        else:
            T_grasp_gripper = gripper.T_grasp_gripper

        T_obj_gripper = self.T_obj_grasp * T_grasp_gripper
    ...
    def grasp_angles_from_stp_z(self, stable_pose):
        ...
        T_stp_grasp = T_stp_obj * T_obj_grasp
    ...
    def find_contacts(self, obj, vis = False, check_approach=True, approach_dist = 0.2, params=None):
        ...
        params : :obj:`dict`
            dictionary of additional parameters (not used here)
        ...
        if obj.sdf is None or (params is not None and 'use_trimesh_contacts' in params.keys() and params['use_trimesh_contacts']):
            return self.find_contacts_trimesh(obj, vis=vis, check_approach=check_approach, approach_dist=approach_dist, params=params)

        return self.find_contacts_sdf(obj, vis=vis, check_approach=check_approach, approach_dist=approach_dist, params=params)

    def find_contacts_sdf(self, obj, vis = False, check_approach=True, approach_dist = 0.2, params=None):
        """ Steps along grasp axis to find the locations of contact with an object
        using ray tracing through the signed distance field.
    ...
    def find_contacts_trimesh(self, obj, vis = False, check_approach=True, approach_dist = 0.2, params=None):
        """ Steps along grasp axis to find the locations of contact with an object
        using Trimesh raytracing
        ...
        # get contact points
        p1, n1 = obj.mesh.find_contact(g1_world, self.axis)
        p2, n2 = obj.mesh.find_contact(g2_world, -self.axis)

        c1_found = (p1 is not None) and (np.linalg.norm(p1-g1_world) < self.open_width/2)
        c2_found = (p2 is not None) and (np.linalg.norm(p2-g2_world) < self.open_width/2)

        # check validity
        c1 = None
        c2 = None
        if c1_found:
            if np.dot(n1, self.axis) > 0:
                n1 = -n1
            c1 = Contact3D(obj, p1, in_direction=self.axis, normal=n1)
        if c2_found:
            if np.dot(n2, -self.axis) > 0:
                n2 = -n2
            c2 = Contact3D(obj, p2, in_direction=-self.axis, normal=n2)
        ...
        while i < num_pts and not contact_found:
            ...
            if on_surface:
                ...
                if i == 0:
                    ...
                    pt_zc = Sdf3D.find_zero_crossing_quadratic(pt_before_before, sdf_before_before, pt_before, sdf_before, pt_grid, sdf_here)
    ...
    def project_camera(self, T_camera_obj, camera_intr):
        """ Project a grasp for a given gripper into the camera specified by a set of intrinsics.

        Parameters
        ----------
        T_camera_obj : :obj:`autolab_core.RigidTransform`
        ...
        T_camera_grasp = T_camera_obj * self.T_obj_grasp
        y_axis_camera = T_camera_grasp.y_axis[:2]
        ...
        t_camera_grasp = T_camera_grasp.translation
        p_camera_grasp = Point(t_camera_grasp, frame=camera_intr.frame)
        u_camera_grasp = camera_intr.project(p_camera_grasp)
        d_camera_grasp = t_camera_grasp[2]
        return gqcnn.Grasp2D(u_camera_grasp, rot_z, d_camera_grasp,
