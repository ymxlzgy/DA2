...
class GraspableObject3D(GraspableObject):
    ...
    def __init__(self, sdf, mesh, key='',
                 ...
                 T_obj_world=RigidTransform(from_frame='obj',
                                            to_frame='world')):
        if sdf is not None and not isinstance(sdf, s.Sdf3D):
    ...
    def rescale(self, scale):
        ...
        sdf_rescaled = None
        if self.sdf_ is not None:
            self.sdf_.rescale(scale)
        mesh_rescaled = self.mesh_.update_tf(stf)
        T_obj_rescaled_world = self.T_obj_world * stf.as_frames(self.T_obj_world.from_frame,
                                                                self.T_obj_world.from_frame)
        ...
        return GraspableObject3D(sdf_rescaled, mesh_rescaled, key=self.key,
                                 ...
                                 T_obj_world=T_obj_rescaled_world)
    ...
    def transform(self, delta_T):
        ...
        mesh_tf = self.mesh_.update_tf(delta_T)
        convex_pieces_tf = None
        T_obj_tf_world = self.T_obj_world * delta_T.inverse().as_frames(self.T_obj_world.from_frame,
                                                                        self.T_obj_world.from_frame)
        if self.convex_pieces_ is not None:
            convex_pieces_tf = []
            for convex_piece in self.convex_pieces_:
    ...
    def surface_information(self, grasp, width, num_steps, plot=False, direction1=None, direction2=None):
       ...
       """
        contacts_found, contacts = grasp.find_contacts(self)#, vis=True)
